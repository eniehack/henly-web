...
export class HostMeta {
	...
	getWebSocketEndpoint(): string | undefined {
		...
		let links = xrd['elements'].filter(
			(elem) => elem['type'] === 'element' && elem['name'] === 'Link'
		);
		let ws_elem = links.find(
			(elem) => elem['attributes']['rel'] === 'urn:xmpp:alt-connections:websocket'
		);
